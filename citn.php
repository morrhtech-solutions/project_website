<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <link rel="stylesheet" href="css/style.css">
    <script src="https://kit.fontawesome.com/27ae433552.js" crossorigin="anonymous"></script>
    <title>Document</title>
</head>
<body>
<?php require "include/header1.php"; ?>
<!-- CITN Slider -->
<div class="atswa_slider_div">
    <img src="images/courses/courses breakdown/citn.png" alt="">
</div>

<div class="schedule_body_for_ATSWA">
<!-- FOUNDATION -->
<div class="schedule_container">
     <h2 class="schedule">SCHEDULE OUTLINE</h2>
     <h3 class="topics">FOUNDATION</h3>
     <ul class="listed_items">
        <li>Principles Of Taxation</li>
        <li>Financial Accounting</li>
        <li>Business Law</li>
     </ul>
     
     <ul class="listed_items">
       <li>Econnomics</li>
     </ul>
     <br>
     <p>*Select and enroll for courses here  <button class="enroll_button">Enroll now</button> </p>
</div>

<!-- PROFESSIONAL TAX I -->
<div class="schedule_container">
     <h3 class="topics">PROFESSIONAL TAX I</h3>
     <ul class="listed_items">
        <li>Financial Reporting</li>
        <li>Income Tax</li>
        <li>Indirect Tax</li>
     </ul>

     <ul class="listed_items">
        <li>Governance Risk And Ethics</li>
     </ul>


     <br>
     <p>*Select and enroll for courses here    <button class="enroll_button">Enroll now</button> <p>
</div>

<!-- PROFESSIONAL TAX II -->
<div class="schedule_container">
     <h3 class="topics">PROFESSIONAL TAX II</h3>
     <ul class="listed_items">
        <li>Tax Audit And Investment</li>
        <li>International Taxation</li>
        <li>Financial / Tax Analysis</li>
     </ul>

     <ul class="listed_items">
        <li>Income Tax For Specialized Business</li>
     </ul>
     <br>
     <p>*Select and enroll for courses here  <button class="enroll_button">Enroll now</button> </p>
</div>

</div>
<!-- CITN Ends-->

<?php require "include/footer.php"; ?>

</body>
</html>